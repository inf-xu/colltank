...
- 统计面板包括：总统计（多少个收藏罐类别，一共多少张图片）。心情统计（每个心情类别的次数）。分类别统计（每个收藏罐类别的图片数量等等）。
- 
...
## 2025-11-17 我的页面统计面板
- 移除原先的设置入口与相关代码，统一由核心偏好模块维护 `app_preferences`。
- 我的页面新增统计卡片，展示收集罐数量、图片总量、本年上传与近7日活跃天数，并自动标记“最满收集罐”。
- 新增分类别统计列表，按收集罐数量排序并提供进度条视觉。
- 我的页面接入 fl_heatmap 构建本年热力图，支持点击查看具体日期上传数量并展示活跃总天数。
- 新增心情统计饼状图，按心情图标聚合数量，展示总记录次数及占比，帮助用户洞察情绪趋势。
